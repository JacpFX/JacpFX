...
public class ApplicationShutdownAndRestartComponentsTest extends AFXSpringLauncher {
    ...
    private static void initStyles() {
        for (int i = 0; i < 2; i++) {
            URL res = ApplicationShutdownAndRestartComponentsTest.class.getResource(BINARY_FILES[i]);
            if (res == null)
                res = ApplicationShutdownAndRestartComponentsTest.class.getResource(STYLE_FILES[i]);
