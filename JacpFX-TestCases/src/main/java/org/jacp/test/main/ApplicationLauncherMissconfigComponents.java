 ...
 */
public class ApplicationLauncherMissconfigComponents extends AFXSpringXmlLauncher {
    private static final Logger log = Logger.getLogger(ApplicationLauncherMissconfigComponents.class
            .getName());
    ...
    public static volatile ApplicationLauncherMissconfigComponents[] instance = new ApplicationLauncherMissconfigComponents[1];

    public ApplicationLauncherMissconfigComponents() {
    }

    public ApplicationLauncherMissconfigComponents(CountDownLatch latch) {
        this.latch = latch;
    }
    ...
    private static void initStyles() {
        for (int i = 0; i < 2; i++) {
            URL res = ApplicationLauncherMissconfigComponents.class.getResource(BINARY_FILES[i]);
            if (res == null)
                res = ApplicationLauncherMissconfigComponents.class.getResource(STYLE_FILES[i]);
