...
public class ApplicationLauncherMoveComponentsBetweenComponents extends AFXSpringLauncher {
    ...
    private static void initStyles() {
        for (int i = 0; i < 2; i++) {
            URL res = ApplicationLauncherMoveComponentsBetweenComponents.class.getResource(BINARY_FILES[i]);
            if (res == null)
                res = ApplicationLauncherMoveComponentsBetweenComponents.class.getResource(STYLE_FILES[i]);
