 ...
 */
public class ApplicationLauncherContextTest extends AFXSpringXmlLauncher {
    private static final Logger log = Logger.getLogger(ApplicationLauncherContextTest.class
            .getName());
    ...
    public ApplicationLauncherContextTest(CountDownLatch latch) {
        this.latch = latch;
    }
    ...
    private static void initStyles() {
        for (int i = 0; i < 2; i++) {
            URL res = ApplicationLauncherContextTest.class.getResource(BINARY_FILES[i]);
            if (res == null)
                res = ApplicationLauncherContextTest.class.getResource(STYLE_FILES[i]);
