 ...
 */
public class ApplicationLauncherMissconfigWorkbench extends AFXSpringXmlLauncher {
    private static final Logger log = Logger.getLogger(ApplicationLauncherMissconfigWorkbench.class
            .getName());
    ...
    private static void initStyles() {
        for (int i = 0; i < 2; i++) {
            URL res = ApplicationLauncherMissconfigWorkbench.class.getResource(BINARY_FILES[i]);
            if (res == null)
                res = ApplicationLauncherMissconfigWorkbench.class.getResource(STYLE_FILES[i]);
