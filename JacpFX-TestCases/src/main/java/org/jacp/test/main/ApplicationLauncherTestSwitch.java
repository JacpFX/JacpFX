...
import org.jacp.test.workbench.WorkbenchTestSwitch;
 ...
 */
public class ApplicationLauncherTestSwitch extends AFXSpringJavaConfigLauncher {
    private static final Logger log = Logger.getLogger(ApplicationLauncherTestSwitch.class
            .getName());
   ...
   // public static volatile ApplicationLauncherTestSwitch[] instance = new ApplicationLauncherTestSwitch[1];
    ...
    protected Class<? extends FXWorkbench> getWorkbenchClass() {
        return WorkbenchTestSwitch.class;
    }
    ...
    private static void initStyles() {
        for (int i = 0; i < 2; i++) {
            URL res = ApplicationLauncherTestSwitch.class.getResource(BINARY_FILES[i]);
            if (res == null)
                res = ApplicationLauncherTestSwitch.class.getResource(STYLE_FILES[i]);
